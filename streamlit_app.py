...
if df_filtered.empty:
    ...
    for i, (_, row) in enumerate(df_filtered.iterrows()):
        ...
        with cols[j]:
            ...
            '>
                <strong style='font-size: 23px;'>📍 {row['ward']}</strong><br>
                🗺️ <strong>{row['outcode']} | {row['district']}</strong><br>
            ...
            if top_crimes:
                content += f"👮<strong>Top 3 Crimes:</strong> {', '.join(top_crimes)}<br>"
            else:
                content += "👮<strong>Top 3 Crimes:</strong> No info<br>"
            ...
            if school_lines:
                for line in school_lines:
                    if ":" in line:
                        ...
                        content += f"🏫 <strong>{label}:</strong>{school_names}<br>"
                    else:
                        content += f"🏫 {line}<br>"
            else:
                content += "🏫 <strong>School:</strong> No info<br>"
